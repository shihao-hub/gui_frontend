    ...
    with ui.column().classes("w-full h-screen"):
        ...
        with ui.row().classes("w-full p-4 bg-gray-100 dark:bg-gray-800 justify-between items-center"):
            with ui.row().classes("items-center gap-4"):
                # todo: dynamic data
                ui.label("主页").classes("text-xl font-bold dark:text-white")
                ui.label("更多物品").classes("dark:text-white font-medium")
                ui.label("宠物增强").classes("dark:text-white font-medium")
        ...
        with ui.row().classes("w-full flex-1 justify-center p-4"):
            ...
            with ui.column():
                # ui.label("饥荒模组集合").classes("text-center")
                # todo: dynamic data
